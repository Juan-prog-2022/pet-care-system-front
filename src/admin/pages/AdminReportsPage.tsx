...
type ReportType = 'appointments' | 'sales'
...
export function AdminReportsPage() {
  ...
  const fetchData = async () => {
    ...
    setError(null)
    try {
      const result = type === 'appointments'
        ? await reportsApi.getAdminAppointments(startDate || undefined, endDate || undefined)
        : await reportsApi.getAdminSales(startDate || undefined, endDate || undefined)
      setData(result)
    } catch {
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, `${type}-reporte.xlsx`)
  }

  const exportPdf = () => {
    if (!data) return
    const doc = new jsPDF()
    const labels = columnLabels[type]
    const columns = data.length > 0 ? Object.keys(data[0]) : []
    const headers = columns.map((c) => labels[c] || formatLabel(c))
    const rows = data.map((row) => columns.map((c) => String(row[c] ?? '')))
    autoTable(doc, { head: [headers], body: rows })
    doc.save(`${type}-reporte.pdf`)
  }

  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-800">Informes</h1>
      ...
      <div className="mb-6 rounded-xl bg-white p-6 shadow-sm">
        <div className="mb-4 flex flex-wrap gap-4">
          <div>
            ...
            <select
              value={type}
              onChange={(e) => { setType(e.target.value as ReportType); setData(null) }}
